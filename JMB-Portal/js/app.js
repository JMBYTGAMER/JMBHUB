...
async function loadServices(){
  ...
  if(firebaseEnabled&&db){try{const snap=await getDoc(doc(db,'settings','portal'));if(snap.exists()&&snap.data().services){services={...services,...snap.data().services}}}catch{}}
  return services;
}
async function boot(){
  const u=await guard();
  ...
  $('#firebaseState').textContent=firebaseEnabled?'Firebase connected':'Demo mode — configure Firebase';
