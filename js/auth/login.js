...
async function startGoogleSignIn(){
  ...
  try{
    ...
    // Mobile browsers are more reliable with Firebase redirect auth.
    // Desktop uses popup first and falls back to redirect if the popup is blocked
    // or the browser reports a popup/cross-origin restriction.
    if(isMobile()){
    ...
    try{
      ...
      const code=e?.code||'';
      if(['auth/popup-blocked','auth/cancelled-popup-request','auth/popup-closed-by-user','auth/internal-error','auth/operation-not-supported-in-this-environment'].includes(code)){
        await signInWithRedirect(auth,provider);
        return;
      }
      throw e;
