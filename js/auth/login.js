...
async function startGoogleSignIn(){
  ...
  try{
    ...
    // Firebase recommends redirect authentication for mobile browsers.
    // The resolver is supplied at call time so Auth does not create its
    // redirect iframe during initial page load.
    if(isMobile()){
    ...
    if(!isMobile() && redirectable.includes(code)){
      try{
        if(label)label.textContent='Redirecting to Google…';
        await signInWithRedirect(auth,provider,browserPopupRedirectResolver);
        return;
