import {auth,backendReady,backendError,functions} from '../core/firebase.js?v=30';
...
forgot.onclick=async()=>{
  ...
  const old=forgot.textContent;forgot.textContent='Sending…';
  try{
    const requestReset=httpsCallable(functions,'requestPasswordReset');
    await requestReset({email:e});
    toast('Check your email for the 6-digit code and Verify now link. The link expires in 5 minutes.','good');
  }catch(err){showError(err?.message?.replace('FirebaseError: ','')||'We could not send the reset email. Please try again later.')}finally{forgot.disabled=false;forgot.textContent=old}
};
